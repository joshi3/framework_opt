...
public class DataDispatcher implements ImsEventDispatcher.VaEventDispatcher {
    ...
    private void handleDefaultBearerDeactivationRequest(int requestId,
        ...
        result = releaseNwRequest(bIsEmergency? PhoneConstants.APN_TYPE_EMERGENCY:
                                                PhoneConstants.APN_TYPE_IMS);
        //removeReceivedAddress(bIsEmergency ? PhoneConstants.APN_TYPE_EMERGENCY:
        //                                                    PhoneConstants.APN_TYPE_IMS);
                        ...
                        if(null != cidArray) {
                            ...
                            for(int i = 0; i < cidArray.length; i++) {
                                notifyDefaultBearerDataConnDeactivated(cidArray[i], nfailCause,
                                    ...
                                    isReasonAllowedToDetach(reason)) {
                            int size = mDeactivateCid.size();
                            log("deactivate cid(s): " + mDeactivateCid + ", size: " + size);
    ...
    private void onNotifyGlobalIpAddr(InetAddress inetIpAddr, String apnType
        ...
        try {
            ...
            ImsAdapter.VaEvent event = mDataDispatcherUtil.composeGlobalIPAddrVaEvent(msgId, cid
    ...
    */
    ...
    private int requestNwRequest(String requestApnType, int phoneId) {
        ...
        if (pos > -1 && pos < endPos) {
            ...
            if (APN_CAP_LIST[pos] != NetworkCapabilities.NET_CAPABILITY_EIMS && EPDG_FEATURE) {
    ...
    private void createNetworkRequest() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            // TODO: is emergency pdn need EPDG ????, tmp skip emergency for WFC
            if (APN_CAP_LIST[i] != NetworkCapabilities.NET_CAPABILITY_EIMS && EPDG_FEATURE) {
    ...
    NetworkCallback mImsNetworkCallback = new NetworkCallback() {
        ...
        public void onLosing(Network network, int maxMsToLive) {
            ...
            log("onAvailable: networInfo: " + getConnectivityManager().getNetworkInfo(network));
            }

            @Override
            public void onLost(Network network) {
            log("onLost: networInfo: " + getConnectivityManager().getNetworkInfo(network));
            getConnectivityManager().unregisterNetworkCallback(mImsNetworkCallback);
                }
