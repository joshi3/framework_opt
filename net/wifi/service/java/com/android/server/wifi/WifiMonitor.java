...
public class WifiMonitor {
    ...
    //M: add
    private static final int NO_CERTIFICATION = 14;
   ...
   /* Network connection completed */
    ...
    ///M: add
     ...
     /** M: NFC Float II @{ */
     public static final int WPS_ER_ENROLLEE_ADD_EVENT            = BASE + 83;
     public static final int WPS_ER_AP_ADD_EVENT                  = BASE + 84;
     public static final int P2P_GO_IP_ALLOCATE_EVENT             = BASE + 86;
     public static final int P2P_GC_IP_GET_EVENT                  = BASE + 87;
     public static final int P2P_NFC_GO_INVITED_EVENT             = BASE + 88;
     /** @} */
    ...
    private static final String P2P_NFC_DEVICE_FOUND_STR = "P2P-NFC-DEVICE-FOUND";
    private static final String P2P_GO_IP_ALLOCATE_STR = "P2P-GO-IP-ALLOCATE";
    private static final String P2P_GC_IP_GET_STR = "P2P-GC-IP-GET";
    private static final String P2P_NFC_GO_INVITED_STR = "P2P-NFC-GO-INVITED";
    /** @} */
    ...
    public boolean isMonitoring() {
        return WifiMonitorSingleton.sInstance.isMonitoring(mInterfaceName);
    }
    ///@}
    ...
    private static class WifiMonitorSingleton {
        ...
        public synchronized void stopMonitoring(String iface) {
            WifiMonitor m = mIfaceMap.get(iface);
            if (m == null) {
                Log.d(TAG, "stopMonitoring iface " + iface + " already gone");
                return;
            }
        ...
        ///M: ALPS01467393: P2pDisableState need stop WifiMonitor @{
        public synchronized boolean isMonitoring(String iface) {
            WifiMonitor m = mIfaceMap.get(iface);
            if (m == null) {
                Log.d(TAG, "isMonitoring iface " + iface + " already gone");
                return false;
            }

            if (DBG) Log.d(TAG, "isMonitoring(" + iface + ") = " + m.mMonitoring);
            return m.mMonitoring;
        }
        ///@}
        ...
        public synchronized void registerInterfaceMonitor(String iface, WifiMonitor m) {
            ...
            Log.d(TAG, "rmIfaceMap put " + iface);
            if (mWifiNative == null) {
                Log.d(TAG, "mWifiNative == null " + iface);
        ...
        public synchronized void killHotspot() {
            //M: add for hotspot manager
            Xlog.d(TAG, "killHotspot!");
            mConnected = false;
        }
        ...
        private synchronized boolean dispatchEvent(String eventStr) {
            ...
            ///M:@{
            WifiMonitor m = null;
            if (iface.equals("unkown0")) {
                ///M: if ap0 exist, it could be hotspot event
                 m = mIfaceMap.get("ap0");
                if (m != null && m.mMonitoring) {
                    //we find hotspot active
                } else {
                    //try p2p0
                    m = mIfaceMap.get("p2p0");
                }
            } else {
    ...
    private boolean dispatchEvent(String eventStr, String iface) {
        ...
        */
        ...
        if ((event == SSID_TEMP_DISABLE)||(event == SSID_REENABLE)) {
            ...
            Log.d(TAG, "Exit because of receiving terminating for ");
            return true;
    ...
    /** M: enhance frequency conflict @{ */
    /* <event> status=<err> and the special case of <event> reason=FREQ_CONFLICT peer_oper_freq=2412*/
    private P2pStatus p2pErrorEx(String dataString) {
        ...
        /* Handle the special case of reason=FREQ_CONFLICT or status=7*/
        if (err == P2pStatus.NO_COMMON_CHANNEL) {
            /* Parsing peer_oper_freq=2412*/
            if (tokens.length > 4) {
                String[] freqNameValue = tokens[4].split("=");
    ...
    private void handleP2pEvents(String dataString) {
        if (dataString.startsWith(P2P_DEVICE_FOUND_STR)) {
            ...
            mStateMachine.sendMessage(P2P_GO_NEGOTIATION_FAILURE_EVENT, sP2pOperFreq, 0, p2pStatus);
            sP2pOperFreq = -1;
            /** @} */
            ...
            P2pStatus p2pStatus = p2pErrorEx(dataString);
            mStateMachine.sendMessage(P2P_GROUP_FORMATION_FAILURE_EVENT, sP2pOperFreq, 0, p2pStatus);
            sP2pOperFreq = -1;
            /** @} */
            ...
            mStateMachine.sendMessage(P2P_GROUP_REMOVED_EVENT, sP2pOperFreq, 0, p2pStatus);
            sP2pOperFreq = -1;
            /** @} */
            ...
            P2pStatus p2pStatus = p2pErrorEx(dataString);
            mStateMachine.sendMessage(P2P_INVITATION_RESULT_EVENT, sP2pOperFreq, 0, p2pStatus);
            sP2pOperFreq = -1;
            /** @} */
        ...
        ///M:@{
            ...
            /*CTRL-EVENT-DISCONNECTED bssid=02:0c:e7:55:55:55 reason=2
              ...
              CTRL-EVENT-DISCONNECTED bssid=02:08:22:0e:e2:00 reason=99 locally_generated=1 peer_oper_freq=2412*/
            String[] tokens = dataString.split(" ");
            ...
            mStateMachine.sendMessage(P2P_PEER_DISCONNECT_EVENT, sP2pOperFreq, 0, nameValue[1]);
            sP2pOperFreq = -1;
            /** @} */
        /** M: NFC Float II @{ */
        } else if (dataString.startsWith(P2P_GO_IP_ALLOCATE_STR)) {
            Log.d(TAG, "handleP2pEvents, send P2P_GO_IP_ALLOCATE_EVENT");
            mStateMachine.sendMessage(P2P_GO_IP_ALLOCATE_EVENT);
        } else if (dataString.startsWith(P2P_GC_IP_GET_STR)) {
            Log.d(TAG, "handleP2pEvents, send P2P_GC_IP_GET_EVENT");
            mStateMachine.sendMessage(P2P_GC_IP_GET_EVENT, dataString);
        } else if (dataString.startsWith(P2P_NFC_GO_INVITED_STR)) {
            mStateMachine.sendMessage(P2P_NFC_GO_INVITED_EVENT, dataString);
        /** } */
        }
        ///@}
    }
    ...
    private void handleHostApEvents(String dataString, String iface) {
        ...
        Log.e(TAG, "handleHostApEvents mWifiNative.mInterfaceName= " + mWifiNative.mInterfaceName + " minitor.iface=" + iface);

        if (tokens[0].equals(AP_STA_CONNECTED_STR)) {
            ///M: modified @{
            if (iface == null) iface = "whatever";
            if (iface.equals("ap0")) {
