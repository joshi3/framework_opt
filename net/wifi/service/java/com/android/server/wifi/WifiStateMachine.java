...
public class WifiStateMachine extends StateMachine {
    ...
    */
    ...
    private static final int WIFI_INDIDE_SOURCE = -7;
    ...
    boolean isRoaming() {
        return mAutoRoaming == WifiAutoJoinController.AUTO_JOIN_ROAMING
                ...
                ///M: ALPS01940840 add driver roaming
                || mAutoRoaming == WifiAutoJoinController.DRIVER_ROMAING;
    }
   ...
   private boolean mIsSupplicantFirstScan = false;
    ...
    private static final boolean mMtkCtpppoe = SystemProperties.get("ro.mtk_ctpppoe_support").equals("1");
    private static final boolean mMtkGeminiSupport = SystemProperties.get("ro.mtk_gemini_support").equals("1");
    ...
    ///M: new bssid maybe driver roaming
            ...
            while (true) {
                while (n < splitData.length) {
                    ...
                    if (splitData[n].equals(END_OF_BATCHES)) {
                        ...
                        logd("retrieveBatchedScanResults X mBatchedScanResults.size()=" + mBatchedScanResults.size());
                        return;
                    }
    ...
    private void handleScreenStateChanged(boolean screenOn, boolean startBackgroundScanIfNeeded) {
        ...
        if (screenOn) {
            ...
            if (getCurrentState() == mConnectedState
                    ...
                    && mAutoJoinScanWhenConnected == true) {
                // Scan after 500ms
  ...
  //      Xlog.d(TAG, "setScanResults, scanResults:" + scanResults);
    ...
    private void fetchRssiLinkSpeedAndFrequencyNative() {
        ...
        Xlog.i(TAG, "fetchRssiLinkSpeedAndFrequencyNative, newRssi:" + newRssi
        ...
        // screen out invalid values
            ...
            Xlog.d(TAG, "mLastSignalLevel:" + mLastSignalLevel
        ...
        if (hasCustomizedAutoConnect()) {
            if (newRssi < IWifiFwkExt.WEAK_SIGNAL_THRESHOLD) {
                ...
                Xlog.d(TAG, "fetchRssi, ip:" + ipAddr
    ...
    boolean shouldSwitchNetwork(int networkDelta) {
        ...
        ///M: don't reconnect wifi if wfd/beamplus/hotknot enable
        if (isTemporarilyDontReconnectWifi()) {
            Xlog.d(TAG, "shouldSwitchNetwork don't switch due to isTemporarilyDontReconnectWifi");
            return false;
        }
        ///M:(Google Issue) don't reconnect wifi because p2p could channel conflict
        if (mTemporarilyDisconnectWifi) {
            Xlog.d(TAG, "shouldSwitchNetwork don't switch due to mTemporarilyDisconnectWifi");
            return false;
        }
        ...
        if (mWifiFwkExt != null && mWifiFwkExt.hasNetworkSelection() != 0
            && mWifiInfo.getNetworkId() != WifiConfiguration.INVALID_NETWORK_ID) {
            Xlog.d(TAG, "hasNetworkSelection Don't");
            return false;
        }
    ...
    class SupplicantStartedState extends State {
        ...
        public void enter() {
            ...
            ///M: use MTK EAP SIM/AKA solution
            mWifiNative.setExternalSim(false);
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                ...
                case M_CMD_SET_POORLINK_RSSI:
                    ...
                    boolean result = mWifiNative.setPoorlinkRssi(value.doubleValue());
                    replyToMessage(message, message.what, (result == true) ? SUCCESS : FAILURE);
                    break;
                case M_CMD_SET_POORLINK_LINKSPEED:
                    value = (Double) message.obj;
                    result = mWifiNative.setPoorlinkSpeed(value.doubleValue());
                    replyToMessage(message, message.what, (result == true) ? SUCCESS : FAILURE);
                    break;
                case M_CMD_GET_TEST_ENV:
    ...
    class DriverStartedState extends State {
        ...
        public void enter() {
            ...
            if (mOperationalMode != CONNECT_MODE) {
                ...
                ///M: don't reconnect wifi if wfd/beamplus/hotknot enable
                if (isTemporarilyDontReconnectWifi()) {
                    Xlog.d(TAG, "disconnect due to isTemporarilyDontReconnectWifi");
                    mWifiNative.disconnect();
                }
            }
            ...
            switch(message.what) {
                ...
                case M_CMD_UPDATE_BGSCAN:
                    if (mWfdConnected.get() ||
                        isTemporarilyDontReconnectWifi()) {
                        stopBatchedScan();
                    } else {
                        startBatchedScan();
                    }
            ...
            switch (message.what) {
                ...
                case WifiMonitor.SUP_REQUEST_IDENTITY:
                    ...
                    if (!hasCustomizedAutoConnect()) {
                        ...
                        Xlog.d(TAG, "Skip SUP_REQUEST_IDENTITY for customization!");
                    }
                    break;
                case WifiMonitor.SUP_REQUEST_SIM_AUTH:
                ...
                case CMD_AUTO_CONNECT:
                    if (hasCustomizedAutoConnect() && !mWifiFwkExt.shouldAutoConnect()) {
                        Xlog.d(TAG, "Skip CMD_AUTO_CONNECT for customization!");
                        return HANDLED;
                    }
                    ...
                    if (hasCustomizedAutoConnect()) {
                        ...
                        tmpResult = mWifiConfigStore.selectNetwork(netId);
                    }
                    if (tmpResult &&
                            mWifiNative.reconnect()) {
                        lastConnectAttempt = System.currentTimeMillis();
                ...
                case WifiManager.START_WPS_REG:
                    ...
                    Xlog.d(TAG, "START_WPS_REG, nfcPw:" + nfcPw);
                    ...
                    if (!TextUtils.isEmpty(wpsInfo.ssid)) {
                        ...
                        wpsResult = new WpsResult(Status.FAILURE);
                        Xlog.e(TAG, "Invalid setup for WPS REG");
                    }
                    if (wpsResult.status == Status.SUCCESS) {
                        replyToMessage(message, WifiManager.START_WPS_SUCCEEDED, wpsResult);
                        transitionTo(mWpsRunningState);
                    } else {
                        Xlog.e(TAG, "Failed to start WPS REG with config " + wpsInfo.toString());
                        replyToMessage(message, WifiManager.WPS_FAILED, WifiManager.ERROR);
                    }
                    break;
                case WifiManager.START_WPS_ER:
                    ...
                    wpsInfo = (WpsInfo) message.obj;
                    Xlog.d(TAG, "START_WPS_ER, mEnrolleeUuid:" + mEnrolleeUuid + ", mEnrolleeBssid:" + mEnrolleeBssid);
                    switch (wpsInfo.setup) {
                        case WpsInfo.PBC:
                        ...
                        default:
                            wpsResult = new WpsResult(Status.FAILURE);
                            Xlog.e(TAG, "Invalid setup for WPS ER!");
                            break;
                    }
                    if (wpsResult.status == Status.SUCCESS) {
                        replyToMessage(message, WifiManager.START_WPS_SUCCEEDED, wpsResult);
                        ...
                        Xlog.e(TAG, "Failed to start WPS ER with config " + wpsInfo.toString());
                        replyToMessage(message, WifiManager.WPS_FAILED, WifiManager.ERROR);
                    }
                    mEnrolleeUuid = null;
                    mEnrolleeBssid = null;
                ...
                case WifiManager.GET_WPS_PIN_AND_CONNECT:   //Item 22
                    ...
                    Xlog.d(TAG, "Item22: GET_WPS_PIN_AND_CONNECT, passwordToken:" + passwordToken);
                    if (!TextUtils.isEmpty(passwordToken) && !passwordToken.equals(UNKNOWN_COMMAND)) {
                        ...
                        if (wpsResult.status == Status.SUCCESS) {
                            ...
                            Xlog.e(TAG, "Failed to start WPS NFC!");
                            replyToMessage(message, WifiManager.WPS_FAILED, WifiManager.ERROR);
                        }
                    } else {
                        Xlog.e(TAG, "Failed to get password token!");
                        replyToMessage(message, WifiManager.WPS_FAILED, WifiManager.ERROR);
                    }
                    break;
                case WifiManager.GET_WPS_CRED_AND_CONNECT:      //Item 26
                    ...
                    Xlog.d(TAG, "Item26: GET_WPS_CRED_AND_CONNECT, credential:" + credential);
                    if (!TextUtils.isEmpty(credential) && !credential.equals(UNKNOWN_COMMAND)) {
                        ...
                        Xlog.e(TAG, "Failed to get WPS credential, mErApUuid:" + mErApUuid);
                        replyToMessage(message, WifiManager.GET_WPS_CRED_AND_CONNECT_FAILED, WifiManager.ERROR);
                    }
                    break;
                case WifiManager.WRITE_PIN_TO_NFC:      //Item 28
                    ...
                    Xlog.d(TAG, "Item28: get requester token, passwordToken: " + passwordToken);
                    if (!TextUtils.isEmpty(passwordToken) && !passwordToken.equals(UNKNOWN_COMMAND)) {
                        ...
                        Xlog.e(TAG, "Failed to get requester token!");
                        replyToMessage(message, WifiManager.WRITE_PIN_TO_NFC_FAILED, WifiManager.ERROR);
                    }
                    break;
                case WifiManager.WRITE_CRED_TO_NFC:     //Item 29
                    ...
                    Xlog.d(TAG, "Item29: get selector token, passwordToken: " + passwordToken);
                    if (!TextUtils.isEmpty(passwordToken) && !passwordToken.equals(UNKNOWN_COMMAND)) {
                        ...
                        Xlog.e(TAG, "Failed to get selector token!");
                        replyToMessage(message, WifiManager.WRITE_CRED_TO_NFC_FAILED, WifiManager.ERROR);
                    }
                    break;
                ...
                case WifiMonitor.WPS_ER_ENROLLEE_ADD_EVENT:
                    ...
                    if (tokens.length >= 3) {
                        ...
                        Xlog.d(TAG, "WPS_ER_ENROLLEE_ADD_EVENT, mEnrolleeUuid:" + mEnrolleeUuid
                        ...
                        if (mWaitingForEnrollee) {
                            ...
                            if (wpsResult.status == Status.SUCCESS) {
                                ...
                                Xlog.e(TAG, "Failed to start WPS ER!");
                            }
                            mEnrolleeUuid = null;
                            mEnrolleeBssid = null;
                        }
                    } else {
                        Xlog.e(TAG, "WPS_ER_ENROLLEE_ADD_EVENT format error, event:" + event);
                    }
                    break;
                case WifiMonitor.WPS_ER_AP_ADD_EVENT:
                    ...
                    tokens = event.split(" ");
                    if (tokens.length < 3) {
                        Xlog.e(TAG, "WPS_ER_AP_ADD_EVENT format error, event:" + event);
                        break;
                    }
                    nfcPw = Settings.System.getInt(mContext.getContentResolver(), "nfc_pw", 0);
                    Xlog.d(TAG, "WPS_ER_AP_ADD_EVENT, erApUuid:" + tokens[1] + ", erApBssid:" + tokens[2]
                ...
                case M_CMD_START_WPS_NFC_TAG_READ:
                    ...
                    if (mErApPin == null) {
                        ...
                        Xlog.d(TAG, "M_CMD_START_WPS_NFC_TAG_READ, mErApPin:" + mErApPin);
                    }
                    break;
                case M_CMD_HS_RECEIVED:
                    if (!mMtkWpsp2pnfcSupport) {
    ...
    class L2ConnectedState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
              ...
              case DhcpStateMachine.CMD_POST_DHCP_ACTION:
                    if (mMtkDhcpv6cWifi) {
                        ...
                        if (message.arg1 == DhcpStateMachine.DHCP_SUCCESS) {
                            ...
                            if (message.arg2 == DhcpStateMachine.DHCPV4) {
                                ...
                                handleSuccessfulIpV6Configuration((DhcpResults) message.obj, M_CMD_DHCP_V6_SUCCESS);
                            }
                ...
                case WifiManager.CONNECT_NETWORK:
                    ...
                    /* Ignore */
                case WifiMonitor.NETWORK_CONNECTION_EVENT:
		    ///M: ALPS01940840 if is new associated bssid, then it is driver roaming @{
		    ...
		    mIsNewAssociatedBssid = false;
                    Xlog.d(TAG, "NETWORK_CONNECTION_EVENT driver roaming");
		    ///@}
                    ///M: poor link
                    ...
                    mLastBssid = (String) message.obj;
                    mWifiInfo.setBSSID(mLastBssid);
                    ///M: ALPS01940840 tag driver roaming
                    mAutoRoaming = WifiAutoJoinController.DRIVER_ROMAING;
                    transitionTo(mObtainingIpState);
                    break;
                case CMD_RSSI_POLL:
    ...
    class VerifyingLinkState extends State {
        ...
        public void enter() {
            ...
            /** M: NFC Float II @{ */
            if (mMtkWpsp2pnfcSupport) {
                Xlog.d(TAG, "Enter VerifyingLinkState, mErApPin:" + mErApPin);
                if (mErApPin != null) {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case CMD_AUTO_ROAM:
                    ...
                    mLastDriverRoamAttempt = 0;
                    if (hasCustomizedAutoConnect() && !mWifiFwkExt.shouldAutoConnect()) {
                        Xlog.d(TAG, "Skip CMD_AUTO_ROAM for customization!");
                        return HANDLED;
                    }
                    /* Connect command coming from auto-join */
    ...
    class DisconnectedState extends State {
        ...
        public void enter() {
           ...
           ///M: modify default mFrameworkScanIntervalMs value @{
            if (hasCustomizedAutoConnect()) {
            ...
            if (hasCustomizedAutoConnect() && mIpConfigLost) {
                Xlog.d(TAG, "IpConfigLost, reconnect!");
                mIpConfigLost = false;
                reconnectCommand();
            }
            ///@}
        ...
        public boolean processMessage(Message message) {
			...
			///M: ALPS01872204 After disabling wifi should go to WaitP2pDisable first @{
                ...
                case M_CMD_UPDATE_BGSCAN:
                    ...
                    if (isTemporarilyDontReconnectWifi()) {
                        Xlog.d(TAG, "isNetworksDisabledDuringConnect:" +
    ...
    class SoftApStartingState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                ...
                case CMD_START_AP_FAILURE:
                    setWifiApState(WIFI_AP_STATE_FAILED);
                    //M: issue ALPS01762105 @{
                    Xlog.d(TAG, "setWifiApEnabled false!");
                    mWifiManager.setWifiApEnabled(null, false);
                    ///@}
                    transitionTo(mInitialState);
                    break;
    ...
    private void initializeExtra() {
                    ...
                    public void onReceive(Context context, Intent intent) {
                        ...
                        if (action.equals(ACTION_CLEAR_WAIT_FLAG)) {
                            ...
                            if (configurationToken != null) {
                                ...
                                Xlog.e(TAG, "No configuration token!");
                            }
                            ...
                            if (passwordToken != null) {
                                ...
                                Xlog.e(TAG, "No password token!");
                            }
                        } else if (action.equals(WifiStateMachine.MTK_WPS_NFC_TESTBED_HR_RECEIVED_ACTION)) {
                            Xlog.d(TAG, "Received HR action, mWaitingForHrToken:" + mWaitingForHrToken);
                            byte[] passwordToken = intent.getByteArrayExtra(WifiStateMachine.MTK_WPS_NFC_TESTBED_EXTRA_PASSWORD);
                            if (passwordToken != null) {
        ...
        mContext.getContentResolver().registerContentObserver(
            Settings.Global.getUriFor(Settings.Global.WIFI_AUTO_JOIN), false,
            new ContentObserver(getHandler()) {
                @Override
                public void onChange(boolean selfChange) {
                    mWifiConfigStore.enableAutoJoinWhenAssociated =
                    ...
                    mWifiConfigStore.enableAutoJoinScanWhenAssociated =
                        Settings.Global.getInt(mContext.getContentResolver(),
                        Settings.Global.WIFI_AUTO_JOIN, 0) == 1;
                    Xlog.e(TAG, "enableAutoJoinWhenAssociated =" +
                        mWifiConfigStore.enableAutoJoinWhenAssociated);
                    Xlog.e(TAG, "enableAutoJoinScanWhenAssociated =" +
                        mWifiConfigStore.enableAutoJoinScanWhenAssociated);
    ...
    private void updateCountryCode(String countryIso) {
        Xlog.e(TAG, "updateCountryCode =" + countryIso);
        if (!TextUtils.isEmpty(countryIso)) {
            Xlog.d(TAG, "countryIso from onCountryDetected:" + countryIso);
            mWifiNative.setCountryCode(countryIso.toUpperCase());
            mCountryCode = countryIso;
        } else {
            Xlog.d(TAG, "cant get countryIso");
        }
    }
    ...
    private void setWfdConnected(WifiDisplayStatus status) {
        ...
        if (featureState == WifiDisplayStatus.FEATURE_STATE_ON
            ...
            Xlog.d(TAG, "mWfdConnected set as true!");
            //mWfdConnected.set(true);
        } else {

            Xlog.d(TAG, "mWfdConnected set as false!");
    ...
    private void showReselectionDialog() {
        mScanForWeakSignal = false;
        Xlog.d(TAG, "showReselectionDialog, mLastNetworkId:" + mLastNetworkId
    ...
    private void checkIfEapNetworkChanged(WifiConfiguration newConfig) {
        Xlog.d(TAG, "checkIfEapNetworkChanged, mLastNetworkId:" + mLastNetworkId
                + ", newConfig:" + newConfig);
        if (newConfig == null) {
            return;
        }
    ...
    private void stopPPPoE() {
        ...
        try {
            ...
            Xlog.d(TAG, "removeNetwork successfully!");
        } catch (Exception e) {
            Xlog.e(TAG, "Exception in removeNetwork:" + e.toString());
        }
        try {
            mNwService.disablePPPOE();
            Xlog.d(TAG, "Stop PPPOE successfully!");
        } catch (Exception e) {
            Xlog.e(TAG, "Exception in disablePPPOE:" + e.toString());
        }
    ...
    private void handleSuccessfulPppoeConfiguration(DhcpResults pppoeResult) {
        ...
        Xlog.d(TAG, "wifiNetId:" + wifiNetId);
        if (wifiNetId != -1) {
            for (RouteInfo route : oldRouteInfos) {
        ...
        try {
            ...
            Xlog.e(TAG, "Exception in config pppoe:" + e.toString());
        }
