...
public class DefaultPhoneNotifier implements PhoneNotifier {
    ...
    private void doNotifyDataConnection(Phone sender, String reason, String apnType,
        ...
        if (SvlteIratUtils.isIratSupportPhone(sender)) {
            if (PhoneConstants.APN_TYPE_DEFAULT.equals(apnType)) {
                log("[IRAT_DPN] Change network type: origin networkType = "
                        + networkType);
            }

            networkType = SvlteIratUtils.getSvltePhoneProxy()
                    .getIratController().getCurrentNetworkType();
        }

        //M: For debug
        if (DBG) {
            networkType = ((telephony!=null) ? telephony.getDataNetworkType(subId) :
