        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(Intent.ACTION_LOCALE_CHANGED)) {
                ...
                if (simState.equals(IccCardConstants.INTENT_VALUE_ICC_ABSENT) ||
                    ...
                    sReceiveNitz[mPhone.getPhoneId()] = false;
...
// M: MTK added end
    ...
    private void setDesireRatMode(int phoneId) {
        ...
        } else if (phoneId == (capabilityPhoneId - 1)) {
        ...
        if (targetNetworkMode >= Phone.NT_MODE_WCDMA_PREF) {
            ...
            log("targetNetworkMode invalid!!");
        }
        log("[setDesireRatMode]-");
    }

    private int getPreferredNetworkModeSettings(int phoneId) {
        int networkType = -1;
        int subId[] = SubscriptionManager.getSubId(phoneId);
        if (subId != null && SubscriptionManager.isValidSubscriptionId(subId[0])) {
            networkType = PhoneFactory.calculatePreferredNetworkType(mPhone.getContext(), subId[0]);
        } else {
            log("Invalid subId, networkType is invalid");
        }
        ...
        if (showPlmn != mCurShowPlmn
            ...
            if (CdmaFeatureOptionUtils.isCdmaLteDcSupport()) {
                if (phoneId >= TelephonyManager.getDefault().getPhoneCount()) {
                    phoneId = Integer.valueOf(
                            SystemProperties.get(PhoneConstants.PROPERTY_CAPABILITY_SWITCH, "1")) - 1;
                }
            }
            if (!mSubscriptionController.setPlmnSpn(phoneId,
    ...
    private void pollStateDone() {
        ...
        log("pollStateDone,hasRegistered:" + hasRegistered + ",hasDeregistered:" + hasDeregistered
                ...
                hasLacChanged + ",sReceiveNitz:" + sReceiveNitz[mPhone.getPhoneId()]);
...
*/
        ...
        if (hasDataRegStateChanged || hasRilDataRadioTechnologyChanged) {
            ...
            if (!SvlteIratUtils.isIratSupportPhone(mPhone)) {
                ...
                if (CdmaFeatureOptionUtils.isCdmaMdIratSupport()) {
                    mPhone.notifyDataConnection(null);
                } else { //M: only for AP IRAT
                    if (SvlteIratUtils.getSvltePhoneProxy().getPsPhone() == mPhone) {
                        mPhone.notifyDataConnection(null);
                    } else {
        ...
        if (hasDataRoamingOff) {
            ...
            //3. From home(domestic) plmn -> roaming URC -> home(domestic) plmn -> recover setup data
    ...
    private void setTimeFromNITZString (String nitz, long nitzReceiveTime) {
        ...
        try {
            ...
            if ((zone == null) || (mZoneOffset != tzOffset) || (mZoneDst != (dst != 0))) {
                ...
                sReceiveNitz[mPhone.getPhoneId()] = true;
            }
    ...
    private void revertToNitzTimeZone() {
        ...
        if (sReceiveNitz[mPhone.getPhoneId()]) {
...
/*
        ...
        if (ar.exception != null || ar.result == null) {
           loge("onPsNetworkStateChangeResult exception");
            ...
            if (mPsRegState != ServiceState.STATE_IN_SERVICE
                ...
                //notify attach
                mSS.setDataRegState(newUrcState);
                mAttachedRegistrants.notifyRegistrants();
                mLastPSRegisteredPLMN = mSS.getOperatorNumeric() ;
            ...
            log("result: " + info[0] + " ,mPsRegState:" + mPsRegState
                + " ,mPsRegStateRaw:" + mPsRegStateRaw);
        }
    }
    ...
    private boolean isSvltePhone() {
        log("isSvltePhone isCdmaLteDcSupport() = "
