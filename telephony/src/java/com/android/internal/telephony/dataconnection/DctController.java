...
public class DctController extends Handler implements IratController.OnIratEventListener {
    ...
    private void updatePhoneBaseForIndex(int index, PhoneBase phoneBase) {
        ...
        if (SvlteIratUtils.isIratSupportPhone(phoneBase)) {
            logd("[IRAT_DctController] Register for RAT events.");
            mIratController = SvlteIratUtils.getSvltePhoneProxy().getIratController();
            mIratDataSwitchHelper = SvlteIratUtils.getSvltePhoneProxy().getIratDataSwitchHelper();
            ...
            mIratDataSwitchHelper.registerForDataConnectionAttached(
                    mRspHandler, EVENT_DATA_ATTACHED + index, null);
            mIratDataSwitchHelper.registerForDataConnectionDetached(
                    mRspHandler, EVENT_DATA_DETACHED + index, null);
            mIratDataSwitchHelper.registerSetDataAllowed(mRspHandler,
    ...
    public void dispose() {
        ...
        for (int i = 0; i < mPhoneNum; ++i) {
            ...
            if (SvlteIratUtils.isIratSupportPhone(phoneBase)) {
                logd("[IRAT_DctController] De-register for RAT events.");
                ...
                mIratDataSwitchHelper.unregisterSetDataAllowed(this);
            }
        }
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case EVENT_CONFIRM_PREDETACH:
                ...
                break;
            // M: [C2K][IRAT] data RAT change to update phone.
    ...
    private int getRequestPhoneId(NetworkRequest networkRequest) {
        ...
        if (SvlteIratUtils.isIratSupportPhone(phoneId)) {
            ...
            logd("[IRAT_DctController] getRequestPhoneId: IRAT change phone ID.");
        }
    ...
    private class TelephonyNetworkFactory extends NetworkFactory {
        ...
        protected void needNetworkFor(NetworkRequest networkRequest, int score) {
            ...
            if (!SubscriptionManager.isUsableSubIdValue(mPhone.getSubId()) || isNetworkRequestSuspend()) {
            ...
            final int requestPhoneId = getRequestPhoneId(networkRequest);
            final int currentPhoneId = mPhone.getPhoneId();
            log("[IRAT_DctController] needNetworkFor: requestPhoneId = "
                    + requestPhoneId + ", currentPhoneId = " + currentPhoneId);
            // M: mapping LTE phone and C2K phone to Phone1 for dual connection.
            if (requestPhoneId == currentPhoneId || (SvlteIratUtils.isIratSupportPhone(mPhone)
                            && requestPhoneId == SvlteIratUtils.getIratSupportSlotId())) {
        ...
        protected void releaseNetworkFor(NetworkRequest networkRequest) {
            ...
            if (SvlteIratUtils.isIratSupportPhone(phoneId)) {
                phoneId = SvlteIratUtils.getIratSupportSlotId();
                logd("[IRAT_DctController] releaseNetworkFor: IRAT change phone ID.");
            }
    ...
    private void transitToAttachingState(int targetPhoneId) {
        ...
        if (topPriorityPhoneId == targetPhoneId
            || (SvlteIratUtils.isIratSupport() && mRequestInfos.isEmpty())) {
            for (int i = 0; i < mDcSwitchStateMachine.length; i++) {
    ...
    private void onIratDataRatChanged(int sourceRat, int targetRat) {
        logd("[IRAT_DctController] onIratDataRatChanged: sourceRat = "
                ...
                + ", targetRat = " + ServiceState.rilRadioTechnologyToString(targetRat));
