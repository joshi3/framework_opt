...
public final class DcTracker extends DcTrackerBase implements IratController.OnIratEventListener {
    ...
    public DcTracker(PhoneBase p) {
        ...
        // M: [C2K][IRAT] Register for IRAT events.
    ...
    protected void registerForAllEvents() {
        if (DBG) log("registerForAllEvents");
    ...
    public void dispose() {
        ...
        // M: [C2K][IRAT] unregister IRAT events.
...
//            attachedState = true;
        ...
        /// M: [C2K][IRAT]Allow data only when data reg state is in service.
        if (allowed && SvlteIratUtils.isIratSupportPhone(mPhone)) {
            allowed &= mIratController.isDrsInService();
            if (!allowed && DBG) {
                log("isDataAllowed: isInservice = " + mIratController.isDrsInService());
            }
        }
        return allowed;
    }
    ...
    private void setupDataOnConnectableApns(String reason, RetryFailures retryFailures) {
        ...
        if (SvlteIratUtils.isIratSupportPhone(mPhone) && mSetupDataBlocked) {
    ...
    protected void notifyOffApnsOfAvailability(String reason) {
        for (ApnContext apnContext : mApnContexts.values()) {
            if ((!mAttached.get() || !apnContext.isReady()) && apnContext.isNeedNotify()) {
                ...
                if (MTK_IMS_SUPPORT) {
                    if ((apnType.equals(PhoneConstants.APN_TYPE_EMERGENCY) &&
                        mPhone.getDataConnectionState(apnType) != PhoneConstants.DataState.DISCONNECTED)
                        ||(apnType.equals(PhoneConstants.APN_TYPE_IMS) && (Phone.REASON_NW_TYPE_CHANGED.equals(reason) ||
                        DctConstants.State.DISCONNECTING == apnContext.getState()))){
    ...
    private boolean isOnlySingleDcAllowed(int rilRadioTech) {
        ...
        if (SystemProperties.get("ro.mtk_gemini_support").equals("1")
                && (SystemProperties.getInt("ril.external.md", 0) - 1) == mPhone.getPhoneId()) {
            if (DBG) {
        ...
        if (TelephonyManager.getDefault().isMultiSimEnabled()) {
            if (mPhone.getPhoneType() == PhoneConstants.PHONE_TYPE_GSM) {
                for (int i = 0; i < phoneCount; i++) {
                    ...
                    if (SvlteIratUtils.isIratSupportPhone(mPhone)
                            && SvlteIratUtils.getIratSupportSlotId() == i) {
    ...
    private boolean getFdnStatus() {
        ...
        if (telephonyEx != null) {
            try {
                bIsFdnEnabled = telephonyEx.isFdnEnabled(mPhone.getPhoneId());
...
*/
    ...
    protected void onDisconnectDone(int connId, AsyncResult ar) {
        ...
        if (mAttached.get() && apnContext.isReady() && retryAfterDisconnected(apnContext)) {
            if (PhoneConstants.APN_TYPE_IMS.equals(apnContext.getApnType()) ||
                ...
                SystemProperties.set(PUPPET_MASTER_RADIO_STRESS_TEST, "false");
                String reason = apnContext.getReason();
                int reconnectTimer = Phone.REASON_APN_CHANGED.equals(reason) ? 3000 : getApnDelay();
                // Wait a bit before trying the next APN, so that
                // we're not tying up the RIL command channel.
                ...
                if(DBG) log("onDisconnectDone: attached, ready and retry after disconnect, reason:"
                            + reason);
    ...
    private void createAllApnList() {
        ...
        if (mAllApnSettings.isEmpty()) {
            ...
            // M: [C2K][IRAT] Set prefer APN for IRAT phone.
            if (SvlteIratUtils.isIratSupportPhone(mPhone)) {
            ...
            } else if (mPreferredApn != null && !mPreferredApn.numeric.equals(operator)) {
                mPreferredApn = null;
                setPreferredApn(-1);
            }
    ...
    public void handleMessage (Message msg) {
        ...
        switch (msg.what) {
            ...
            case SvlteIratUtils.EVENT_IRAT_DATA_RAT_CHANGED:
                ...
                break;
            case SvlteIratUtils.EVENT_LTE_RECORDS_LOADED:
                onLteRecordsLoaded();
                break;
            case SvlteIratUtils.EVENT_RETRY_SETUP_DATA_FOR_IRAT:
    ...
    private IccRecords getUiccRecords(int appFamily) {
        ...
        if (!SvlteIratUtils.isIratSupportPhone(mPhone)) {
            ...
            return mUiccController.getIccRecords(SvlteIratUtils.getIratSupportSlotId(), appFamily);
        }
    }
    ...
    protected void onUpdateIcc() {
        ...
        if (SvlteIratUtils.isIratSupportPhone(mPhone)
                ...
                && newIccRecords != null) {
            log("[IRAT_DcTracker] Register for LTE IccRecords loaded.");
            mLteIccRecords.set(newIccRecords);
            newIccRecords.registerForRecordsLoaded(this,
                    SvlteIratUtils.EVENT_LTE_RECORDS_LOADED, null);
        }
    ...
    private void onIratDataRatChanged(int sourceRat, int targetRat) {
        ...
        if (targetRat == ServiceState.RIL_RADIO_TECHNOLOGY_UNKNOWN) {
            ...
            updatePhoneIfNeeded(sourceRat, targetRat);
            ...
            Message disconnectMessage = Message.obtain(this,
                    SvlteIratUtils.EVENT_RETRY_SETUP_DATA_FOR_IRAT);
    ...
    public void onIratEnded(Object info) {
        ...
        updatePhoneIfNeeded(mdIratInfo.sourceRat, mdIratInfo.targetRat);
    ...
    private ApnSetting makeInitialAttachApn() {
        ...
        if (operator != null) {
            ...
            if (cursor != null) {
                if (cursor.getCount() > 0) {
                    iaApnSetting = createApnList(cursor).get(0);
                }
                cursor.close();
            }
        }
    ...
    /**
     * Called when IRAT_SUCCEED,we should change the phone.
     * @param sourceRat source radio technology
     * @param targetRat target radio technology
     */
    private void updatePhoneIfNeeded(int sourceRat, int targetRat) {
        PhoneBase newPsPhone = (PhoneBase) (SvlteIratUtils.getSvltePhoneProxy())
                .getPsPhone();
        log("[IRAT_DcTracker]updatePhoneIfNeeded: sourceRat=" + sourceRat
                + ", targetRat=" + targetRat + ", mPhone = " + mPhone
                + ", newPhone=" + newPsPhone);
    ...
    private IRilDcArbitrator getRilDcArbitrator() {
        return SvlteIratUtils.getSvltePhoneProxy().getRilDcArbitrator();
    }
    ...
    private void updateIccRecord(int targetRat) {
        ...
        final int family = SvlteIratUtils.getUiccFamilyByRat(targetRat);
        IccRecords newIccRecords = getUiccRecords(family);
        IccRecords r = mIccRecords.get();
    ...
    private void registerForIratEvents() {
        if (SvlteIratUtils.isIratSupportPhone(mPhone)) {
            mIratController = SvlteIratUtils.getSvltePhoneProxy()
                    .getIratController();
            mIratController.registerForRatChanged(this,
                    SvlteIratUtils.EVENT_IRAT_DATA_RAT_CHANGED, null);
            mIratController.addOnIratEventListener(this);

            mIratDataSwitchHelper = SvlteIratUtils.getSvltePhoneProxy()
                    .getIratDataSwitchHelper();
            ...
            mSstProxy.registerForPsRestrictedEnabled(this,
                    DctConstants.EVENT_PS_RESTRICT_ENABLED, null);
            mSstProxy.registerForPsRestrictedDisabled(this,
                    DctConstants.EVENT_PS_RESTRICT_DISABLED, null);
            log("[IRAT_DcTracker] registerForIratEvents: mIratController = "
                    + mIratController + ", mIratDataSwitchHelper = "
                    + mIratDataSwitchHelper + ", mSstProxy = " + mSstProxy);
        }
    }
    ...
    private void unregisterForIratEvents() {
        if (SvlteIratUtils.isIratSupportPhone(mPhone)) {
            ...
            mSstProxy.unregisterForPsRestrictedEnabled(this);
            mSstProxy.unregisterForPsRestrictedDisabled(this);
            IccRecords r = mLteIccRecords.get();
            if (r != null) {
                r.unregisterForRecordsLoaded(this);
    ...
    private String getPreferApnByRat(int rat) {
        final int family = SvlteIratUtils.getUiccFamilyByRat(rat);
    ...
    private boolean isCdmaFallbackCase(int sourceRat, int targetRat) {
        final int sourceFamily = SvlteIratUtils.getUiccFamilyByRat(sourceRat);
        final int targetFamily = SvlteIratUtils.getUiccFamilyByRat(targetRat);
    ...
    private int getUiccFamily(PhoneBase phone) {
        ...
        if (!SvlteIratUtils.isIratSupportPhone(phone)) {
            ...
            if (radioTech != ServiceState.RIL_RADIO_TECHNOLOGY_UNKNOWN) {
                family = SvlteIratUtils.getUiccFamilyByRat(radioTech);
            }
