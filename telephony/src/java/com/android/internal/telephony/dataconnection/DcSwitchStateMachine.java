        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                case DcSwitchAsyncChannel.REQ_CONNECT: {
                    ...
                    /// M: [C2K][IRAT] Set PS active slot when data detached.
                    if (SvlteIratUtils.isIratSupport()) {
                        log("REQ_CONNECT : mId = " + mId);
                        setPsActiveSim(mId, null);
                    }
...
*/
        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                case DcSwitchAsyncChannel.REQ_CONNECT: {
                    ...
                    // M: [C2K][IRAT] Change the format of RIL request
                    setDataAllowed(false, obtainMessage(
    ...
    private void setPsActiveSim(int phoneId, Message result) {
        SvlteIratUtils.getSvltePhoneProxy().getRilDcArbitrator().requestSetPsActiveSlot(
                phoneId + 1, null);
    }

    private void setDataAllowed(boolean allowed, Message result) {
        if (SvlteIratUtils.isIratSupportPhone(mPhone)) {
            SvlteIratUtils.getSvltePhoneProxy().getIratDataSwitchHelper()
