...
public class ImsService extends IImsService.Stub {
    ...
    /// @}
    public ImsService(Context context, CommandsInterface[] ci) {
        mImsAdapter = new ImsAdapter(context);
        mCommandsInterfaceArray = ci;
        if (ci != null) {
            mImsRILAdapter = new ImsRILAdapter(context, ci[0]);
        } else {
            Rlog.w(LOG_TAG, "IMS: something wrong in ImsService constructor, ci = " 
                + ci);
        }
    ...
    public void turnOnIms(int phoneId) {
        ...
        // IMS on/off, re-connect IMS Service to right commands interface of GsmPhone.
        if (isPhoneIdChanged && mCommandsInterfaceArray != null && mActivePhoneId >= 0
                && mActivePhoneId < mCommandsInterfaceArray.length) {
            Rlog.d(LOG_TAG, "IMS: updateImsRilAdapterCi !!");
            mImsRILAdapter.updateImsRilAdapterCi(
                    mCommandsInterfaceArray[mActivePhoneId]);
        }
    ...
    public void notifySrvccDone() {
        if (mImsRILAdapter == null) {
            Rlog.e(LOG_TAG, "IMS: notifySrvccDone error, mImsRILAdapter is null ");
        }

        mImsRILAdapter.notifySrvccDone();
    }
