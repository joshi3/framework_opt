...
public class ImsCallSessionProxy extends IImsCallSession.Stub {
    ...
    private boolean mIsCallHold = false;
    ...
    public void hold(ImsStreamMediaProfile profile) {
        if (mIsCallHold) {
            ...
            mHandler.sendMessage(mHandler.obtainMessage(EVENT_HOLD_RESULT, ar));
        } else {
    ...
    public void resume(ImsStreamMediaProfile profile) {
        if (mIsCallActive) {
            AsyncResult ar = new AsyncResult(null, null, null);
            mHandler.sendMessage(mHandler.obtainMessage(EVENT_RESUME_RESULT, ar));
        } else {
    ...
    public void merge() {    
        ...
        if (myCallInfo.mIsConference == false && beMergedCallInfo.mIsConference == false) {
            ...
            if (myCallInfo.mIsConference && myCallInfo.mState == ImsCallInfo.State.HOLDING) {
                ...
                if (myCallInfo.mState == ImsCallInfo.State.ACTIVE) {
                    //swap current call to hold call before merging
                    mImsRILAdapter.hold(result);
                } else {
                    //swap current call to active call before merging
                    mImsRILAdapter.resume(result);
                }
            }
        } else {
        ...
        private void handleImsConfCallMessage(int len, int callId, String data) {
            try {
                ...
                for (ConferenceCallMessageHandler.User u : users) {
                    ...
                    confInfo.putString(ImsConferenceState.USER, u.getEntity());
                    ...
                    if (mIsMerging && (!callInfo[0].equals(mCallId))) {
                        ...
                        switch (msgType) {
                            ...
                            case 131: //CSMCC_STATE_CHANGE_HELD
                                ...
                                if(mListener != null) {
                                    try {
                                        mListener.callSessionHoldReceived(ImsCallSessionProxy.this,
                                                mCallProfile);
                                    } catch (RemoteException e) {
                            ...
                            case 132: //CSMCC_STATE_CHANGE_ACTIVE
                                ...
                                if (mListener != null) {
                                    try {
                                        if (mOnHold == true) {
                                            mListener.callSessionResumeReceived(
                                                    ImsCallSessionProxy.this, mCallProfile);
                                        }
                                    } catch (RemoteException e) {
                                        Rlog.e(LOG_TAG, "RemoteException SessionResumeReceived");
                                    }
                                }
                                break;
                ...
                case EVENT_RESUME_RESULT:
                    ...
                    if (mListener != null) {
                        if (ar.exception != null) {
                            ...
                            try {
                                mListener.callSessionResumed(ImsCallSessionProxy.this,
                                        mCallProfile);
                                mOnHold = false;
                            } catch (RemoteException e) {
                                Rlog.e(LOG_TAG, "RemoteException callSessionResumed()");
                            }
                        }
                    }
                    break;
